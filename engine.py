...
def train_one_epoch(
    ...
    Args:
        ...
        criterion: function that takes outputs and targets and return the loss
        data_loader: torch.utils.data.DataLoader that iterates over the dataset
        optimizer: torch.optim.Optimizer for training
        device: device where the tensors are
        epoch: current epoch (used for lr scheduling)
        max_norm: if > 0, will normalize the gradients to have a maximum L2 norm equal to max_norm
        model_ema: if not None, will update the ema model parameters
    """
    model.train()
    ...
    for i, batch_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if args.masks:
            ...
            # print("engine else ", memory_cache["tokenized"]._encodings)
            outputs, memory_cache = model(samples, captions, encode_and_save=False, memory_cache=memory_cache, combine_encode=True)
...
def evaluate(
    ...
    Args:
        ...
        contrastive_criterion: the loss function for contrastive learning
        qa_criterion: the loss function for question answering
        postprocessors: the postprocessors to apply to the model's output
        weight_dict: the weights for the losses
        data_loader: the validation dataset
        evaluator_list: the list of evaluators to use
