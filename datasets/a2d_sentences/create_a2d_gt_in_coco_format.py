...
from datasets.a2d_sentences.utils import get_image_id
...
def create_a2d_sentences_ground_truth_test_annotations(subset_type, dataset_path, output_path):
    ...
    for annot in tqdm(text_annotations):
        ...
        for p in annot_paths:
            ...
            image_id = get_image_id(video_id=video_id, frame_idx=frame_idx, ref_instance_a2d_id=instance_id)
    ...
    print(f'--------- Set: {subset_type} ---------')
    print(f'Number of annotations: {len(annotations_dict)}')
    print(f'Number of images: {len(images_dict)}')
    print(f'---------------------------')
...
if __name__ == '__main__':
    ...
    args.output_json_path =  path.join(output_path, f'a2d_sentences_{args.subset_type}_annotations_in_coco_format.json')
    print(args)
    create_a2d_sentences_ground_truth_test_annotations(subset_type=args.subset_type, dataset_path=args.dataset_path, output_path=args.output_json_path)